...
docker compose exec mm python /app/cli/mysql-cli.py mysql start-cluster --config-file "/etc/mm/config-mysql-1.ini"
...
docker compose exec mysql-s1 mysql -udbadmin -ppassword -h proxysql -e "select * from sales.t1;"
